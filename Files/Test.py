...
class User:
    def __init__(self, username, system_username, password, is_manager=False):
...
class MainWindow(QWidget):
    def __init__(self, user):
        super().__init__()
        self.user = user
        self.setWindowTitle("Survey Dashboard")
        ...
        user_layout.addWidget(QLabel(f"Username: {user.username}"))
        user_layout.addWidget(QLabel(f"System: {user.system_username}"))
        user_group.setLayout(user_layout)
        nav_layout.addWidget(user_group)
        
        # Navigation buttons
        self.feedback_btn = QPushButton("Feedback")
        self.score_btn = QPushButton("Score Chart")
        style_buttons(self.feedback_btn)
        style_buttons(self.score_btn)
        
        nav_layout.addWidget(self.feedback_btn)
        nav_layout.addWidget(self.score_btn)
        ...
        self.score_btn.clicked.connect(self.show_scores)
        
        # Initially hide score button if user has less than 5 rows
        self.update_score_button_visibility()
    
    def create_direct_tab(self):
        tab = QWidget()
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Direct Reports"))
        self.direct_canvas = MatplotlibCanvas()
        layout.addWidget(self.direct_canvas)
        tab.setLayout(layout)
        self.content_area.addTab(tab, "Direct Reports")
    
    def create_indirect_tab(self):
        tab = QWidget()
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Indirect Reports"))
        self.indirect_canvas = MatplotlibCanvas()
        layout.addWidget(self.indirect_canvas)
        tab.setLayout(layout)
        self.content_area.addTab(tab, "Indirect Reports")
    
    def update_score_button_visibility(self):
        ...
        self.score_btn.setVisible(count >= 5 or self.user.is_manager)
    
    def show_feedback(self):
        self.survey_app = SurveyApp()
        self.survey_app.show()
...
class AuthWindow(QWidget):
    def __init__(self):
        ...
        style_buttons(login_btn)
        style_buttons(signup_btn)
    ...
    def login(self):
        ...
        cursor.execute('''SELECT * FROM users 
                         WHERE username=? AND password=? AND system_username=?''', 
                      (username, password, system_user))
        user = cursor.fetchone()
        conn.close()
        
        if user:
            self.hide()
            self.main_window = MainWindow(User(username, system_user, password, user[3]))
            self.main_window.show()
        else:
            QMessageBox.warning(self, "Error", "Invalid credentials or wrong system")
...
class SignupWindow(QWidget):
    def __init__(self):
        ...
        self.password = QLineEdit()
        self.password.setEchoMode(QLineEdit.EchoMode.Password)
        self.is_manager = QCheckBox("Sign up as manager")
        ...
        layout.addWidget(QLabel("Password:"))
        layout.addWidget(self.password)
        layout.addWidget(self.is_manager)
        layout.addWidget(signup_btn)
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS users
                         (username TEXT, system_username TEXT, 
                          password TEXT, is_manager INTEGER)''')
        conn.close()
    ...
    def create_account(self):
        ...
        password = self.password.text()
        system_user = os.getlogin()
        is_manager = 1 if self.is_manager.isChecked() else 0
        
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute('INSERT INTO users VALUES (?, ?, ?, ?)', 
                      (username, system_user, password, is_manager))
        conn.commit()
        conn.close()
